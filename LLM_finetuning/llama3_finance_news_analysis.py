...
# 이 파일의 학습 데이터는 샘플마다 길이가 제각각(918~5,903 토큰)이라서 매 스텝 요구하는
...
if torch.cuda.is_available():
    ...
    ##                     -> 세 모델 중 이 파일에서 효과가 가장 큽니다 (2GB 절약)
    ##
    ## 바로 위 prepare_model_for_kbit_training 은 학습 안정성을 위해
    ## "4비트가 아닌 모든 파라미터"를 float32로 승격시킵니다. 문제는 이 모델에서
    ## 4비트가 아닌 파라미터의 거의 전부가 단어 임베딩과 lm_head 라는 점입니다.
    ##
    ## 게다가 Llama-3 계열은 tie_word_embeddings=False 라서 이 둘이 가중치를 공유하지 않고
    ## "따로따로" 존재합니다. 즉 같은 크기의 거대한 행렬이 두 개입니다.
    ##   embed_tokens  128,256 x 4,096 = 525,336,576개
    ##   lm_head       128,256 x 4,096 = 525,336,576개
    ##   합계 1,050,673,152개 (= 10.5억개, 8B 모델 전체의 13%)
    ##     bf16(2바이트)    -> 2,004 MB
    ##     float32(4바이트) -> 4,007 MB   <- 그냥 두면 이쪽
    ##
    ##   참고로 kanana-2 와 Konan-LLM-OND 는 두 층이 가중치를 공유(tied)해서 행렬이 하나뿐입니다.
    ##   같은 처리를 해도 절약되는 양이 313MB / 844MB 수준인데, 이 파일만 2GB 입니다.
    ##
    ## 2GB 차이도 크지만 진짜 문제는 따로 있습니다.
    ...
    ## 1차원 파라미터(LayerNorm 등)는 float32로 남겨두는데, 이건 QLoRA 논문이 권장하는 방식입니다.
...
#
#   문장 하나가 5,903 토큰이고 사전이 128,256개라면
#     5,903 x 128,256 = 약 7.6억 개의 숫자
#   손실 계산은 정밀도를 위해 float32(숫자 1개당 4바이트)로 하므로   ->   2.82 GB
#
...
#   => 2.82GB x 3 = 약 8.5 GB
#
# QLoRA가 줄여주는 것은 오직 "가중치"뿐이며, 활성화(activation)와 로짓은 1비트도 줄여주지 못합니다.
#
#   이 모델의 실제 메모리 구성 (문장 길이 5,903 기준)
#     4비트로 압축된 본체 (약 6,979M 파라미터)              3,328 MB
#     embed_tokens + lm_head (10.5억개, 서로 분리되어 있음) 2,004 MB (bf16) / 4,007 MB (float32)
#     LoRA + AdamW 옵티마이저 상태                             수십 MB
#     로짓 3벌                                             8,460 MB   <- 압도적 1위
#
#   손대지 않은 상태(float32 임베딩 + nll)의 합계는 약 16GB 로 12GB GPU 에 애초에 들어가지 않습니다.
#   아래 해결책을 모두 적용하면 약 7GB 수준으로 내려가 12GB GPU 에서 돌아갑니다.
#
# [초보자가 빠지기 쉬운 함정]
#   "파라미터 수"만 보고 메모리를 예측하면 반드시 틀립니다. 메모리를 결정하는 것은
#     (a) 파라미터 수  (b) 사전 크기(vocab_size)  (c) 문장 길이  (d) 배치 크기
#   네 가지이고, QLoRA는 이 중 (a) 하나만 줄여줍니다.
#   그래서 8B 모델이든 1.3B 모델이든 (b)(c)(d)가 같으면 로짓 메모리는 똑같이 듭니다.
#   실제로 같은 폴더의 kanana-2-1.3b 는 파라미터가 1/6 인데도 로짓 크기는 문장 길이 비율만큼만 차이 납니다.
#
# -----------------------------------------------------------------------------------------------------
# 2. "초반엔 괜찮다가 중반에 터지는" 이유
# -----------------------------------------------------------------------------------------------------
# 로짓 크기는 문장 길이에 정비례하는데, 이 데이터셋의 문장 길이는 제각각입니다.
# (아래 max_seq_length 주석의 실측 분포 참고: 최소 918 ~ 최대 5,903 토큰)
#
# 학습은 데이터를 무작위 순서로 도는데, 운 좋게 짧은 문장이 이어지면 조용히 돌다가
# 긴 문장을 만나는 순간 훅 튑니다. "중반부터 터진다"가 아니라 "긴 문장을 만나면 터진다"가 정확합니다.
# 이 파일의 데이터는 세 모델 중 길이 편차가 가장 커서(918~5,903, 6.4배) 이 현상이 가장 심합니다.
#
# 여기에 "메모리 단편화"가 겹칩니다.
# 2.82GB짜리 로짓은 반드시 "연속된" 메모리 덩어리여야 하는데, 매 스텝 요구 크기가 달라지다 보니
...
#   [2] QLoRA 분기에서 임베딩/lm_head 를 bf16 으로 되돌림 -> 2GB 절약 + 활성화 메모리 절반 (위 CUDA 분기 안)
#   [3] max_seq_length 를 8192 -> 6144                 -> 최악의 경우 상한을 못박음 (바로 아래)
...
# 훈련 데이터 793개를 이 모델(Llama-VARCO-8B-Instruct)의 토크나이저로 재보고,
...
#   4                        2,484                   2,484  <- 신기록
#   10                       4,772                   4,772  <- 신기록
#   159                      5,717                   5,717  <- 신기록   (진행표에는 158로 보임)
#   169                      5,903                   5,903  <- 신기록   (진행표에는 168로 보임)
#
...
# -----------------------------------------------------------------------------------------------------
#
#   길이 분포 : min 918 / 중앙값 1,563 / 평균 1,646 / p90 2,234 / p99 3,222 / 최대 5,903
#   상위 12개 : 3,066 3,150 3,166 3,218 3,268 3,345 3,435 3,556 3,849 │ 4,772 5,717 5,903
#                                                                      └ 여기서 뚝 끊깁니다
#
#   길이 초과 기준       샘플 수      비율
#   --------------------------------------
...
#   128 배수로 버킷팅하면 모양이 25 종류인데, 모양마다 활성값 덩어리 한 벌이 캐시에 따로 남습니다.
#   25 x 약 2GB = 약 50GB 로, 위 표의 천장 40.5GB 와 맞아떨어집니다.
#
...
#   켰을 때 40.5GB -> 28.7GB 였습니다. 대신 계산이 16% 늘어납니다)
#
# [가장 단순한 길] 아래 DROP_SAMPLES_LONGER_THAN 을 켜면 유난히 긴 샘플 몇 개를 아예 빼버립니다.
#   이 모델 기준 4096 토큰 초과는 3개뿐이고, 빼고 나면 최대 길이가 3,849 토큰으로 떨어져
...
#
# Llama-VARCO 토크나이저로, 그리고 아래 collate_fn 이 실제로 만드는 형식 그대로
# 데이터 991개를 전부 실측한 분포:
#   최소 918 / 중앙값 1,559 / 평균 1,631 / 최대 5,903 토큰
#   (p90 = 2,196  p95 = 2,484  p99 = 3,197)
#
#   max_seq_length   잘리는 샘플      정답까지 통째로 사라지는 샘플   최대 로짓 1벌
#   ---------------------------------------------------------------------------------
#        2048        150개 (15.1%)           83개                   0.98 GB
#        3072         12개 ( 1.2%)            7개                   1.47 GB
#        4096          3개 ( 0.3%)            3개                   1.96 GB
#        6144          0개 ( 0.0%)            0개                   2.82 GB  <- 이 파일이 쓰는 값
#        8192          0개 ( 0.0%)            0개                   2.82 GB  <- 예전 값
#
# 예전 값 8192는 실제 최대 길이(5,903)보다 커서 사실상 아무 일도 하지 않는 값이었습니다.
# 6144로 낮추면 잘리는 샘플은 여전히 0개인데 "최악의 경우 얼마나 쓸지"의 상한이 확실히 못박힙니다.
#
# !! [경고] 이 파일 아래쪽 "맥북 학습 시간" 주석은 max_seq_length=2048 을 권하고 있는데,
#    위 표를 보면 그 값에서는 991개 중 150개(15.1%)가 잘리고, 그 중 83개는 정답이 통째로 사라집니다.
#    정답이 사라지면 아래 collate_fn 이 assistant 헤더를 찾지 못해 labels 가 전부 -100 이 되고,
#    학습 대상 토큰이 0개인 배치의 cross entropy 는 NaN 이 되어 그 스텝에서 모델이 통째로 망가집니다.
#    (자세한 내용은 아래 "[알아두면 좋은 점 2: 잘림(truncation)이 발생하면 여기서 사고가 난다]" 참고)
#
#    2048 을 쓰고 싶다면 반드시 학습 전에 긴 샘플을 걸러내야 합니다.
#    메모리만 놓고 보면 chunked_nll 이 켜져 있는 지금은 6144 로 두어도 문제가 없습니다.
# -----------------------------------------------------------------------------------------------------
max_seq_length = 6144
...
## 배치마다 가장 긴 문장에 딱 맞춰 패딩하면 텐서 모양이 618종류나 생기고, 모양마다 활성값 덩어리
## 한 벌이 캐시에 따로 남습니다. 128 의 배수로 맞추면 모양이 25종류로 줄고 계산 낭비는 3.9%뿐입니다.
...
args = SFTConfig(
    ...
    ##
    ## 이 모델(Llama-VARCO-8B)은 32개 층 전부가 일반 full attention 이라 마스크가 None 으로 넘어가고
    ## FlashAttention 커널을 탈 수 있어서 kanana2 만큼 극적이지는 않습니다. 하지만 8B 모델에서
    ## 32개 층의 모든 중간 계산값을 backward 까지 들고 있는 것은 애초에 선택지가 아닙니다.
    ## (이 파일 아래쪽 맥북 최적화 주석에도 끄면 8.6배 느려지거나 OOM 났다는 실측이 있습니다)
    ...
    ## "로짓 3벌 = 8.5GB" 문제를 정면으로 해결하는 옵션입니다.
    ##
    ...
    ##   이 파일의 데이터는 최대 5,903 토큰으로 1.5배 더 기니 nll 쪽 손해도 그만큼 더 큽니다.
    ##
    ## TRL 1.12 부터 이미 기본값이지만, 눈에 보이도록 명시해 둡니다.
    ## !! 구버전 TRL 에는 이 옵션이 없어서 TypeError 가 납니다. 그러면 TRL 을 업그레이드하세요.
    ##    (8B 모델을 12GB GPU에서 돌리려면 이 옵션이 사실상 필수라, 업그레이드를 권합니다)
    ...
    ##       최소 918 / 중앙값 1,559 / 평균 1,631 / 최대 5,903  (collate_fn 이 만드는 형식 그대로 실측)
    ##       1024 이하  57개 ( 5.8%)
    ##       2048 이하 839개 (84.7%)
...
def collate_fn(batch):
    ...
    for example in batch:
        ...
        ##
        ## 이 데이터셋의 토큰 길이 분포는 최소 918 / 중앙값 1,559 / 최대 5,903 이고 max_seq_length=6144 이므로,
        ## 잘리는 샘플이 0개라서 지금은 이 함정에 걸리지 않습니다.
        ## 하지만 메모리를 아끼려고 값을 낮추는 순간 바로 위험해집니다. (max_seq_length 주석의 표 참고)
        ##   4096으로 낮추면   3개가 잘리고 그 중  3개는 정답이 통째로 사라집니다.
        ##   2048로 낮추면  150개가 잘리고 그 중 83개는 정답이 통째로 사라집니다.  <- 데이터의 8%가 NaN 폭탄
        ## 값을 낮출 거라면 학습 전에 길이가 max_seq_length를 넘는 샘플을 데이터셋에서 미리 걸러내십시오.
        i = 0
...
# 아래 값을 4096로 바꾸면 그 3개를 빼고 학습합니다.
#   - 최대 길이가 6144 -> 3,849 토큰으로 떨어져 159 / 169 step의 계단이 통째로 사라집니다.
#   - 대신 학습 데이터의 0.4%를 버리게 됩니다.
#
# 데이터를 버리는 선택이라 기본값은 None(끔)입니다. 위 두 대책(캐시 반납 + 버킷팅)으로도
# 메모리가 부족할 때 마지막 수단으로 켜세요.
# =====================================================================================================
DROP_SAMPLES_LONGER_THAN = None  # 예: 4096
...
#   항목                                          계산식                            크기
#   ------------------------------------------------------------------------------------
#   ① 4bit 양자화된 본체 가중치                   6,979M x 0.5B                3,328 MiB
#   ② 양자화 상수 (double quant)                  6,979M x 0.127bit              106 MiB
#   ③ 임베딩 + lm_head (bf16, 각각 별도)          1,051M x 2B                  2,004 MiB
#   ④ LayerNorm 등 1차원 파라미터 (fp32 유지)     266K x 4B                      1.0 MiB
#   ⑤ LoRA 가중치 (fp32)  <- 유일한 학습 대상     3,407,872 x 4B                13.0 MiB
#   ⑥ LoRA 그레디언트 (fp32)                      3,407,872 x 4B                13.0 MiB
#   ⑦ AdamW 옵티마이저 상태 m, v (fp32)           3,407,872 x 8B                26.0 MiB
#   ------------------------------------------------------------------------------------
#   A 소계                                                                     5,491 MiB
#
# [여기가 가장 놀라운 지점입니다]
# 보통 학습 메모리의 주범으로 꼽히는 ⑥그레디언트 + ⑦옵티마이저 상태가 합쳐서 겨우 39 MiB 입니다.
# LoRA 가 전체 파라미터의 0.042% 만 학습하기 때문입니다.
# 풀 파인튜닝이었다면 이 두 항목만으로 8.03B x 12바이트 = 약 89.7 GiB 가 필요했을 겁니다.
#   (그레디언트 fp32 4B + Adam 의 1차 모멘트 m 4B + 2차 모멘트 v 4B = 파라미터당 12바이트)
# "QLoRA 는 가중치를 4비트로 줄이는 기법"이라고들 하지만, 실제로 가장 크게 아끼는 것은 ①이 아니라 ⑥⑦입니다.
#
# ③을 fp32 로 두면(= 위 CUDA 분기에서 bf16 으로 되돌리지 않으면) 4,008 MiB 가 되어 2,004 MiB 손해입니다.
#
# -----------------------------------------------------------------------------------------------------
# B. 스텝마다 변하는 메모리 — 문장 길이 S 에 비례
# -----------------------------------------------------------------------------------------------------
#
# 이 데이터셋의 실측 길이: 최소 918 / 중앙값 1,559 / 최대 5,903 토큰
#
#   항목                                          계산식                         S=1,559      S=5,903
#   -------------------------------------------------------------------------------------------------
#   ⑧ 체크포인트 저장분 (32개 층의 입력)          S x 262,144B                   390 MiB    1,476 MiB
#   ⑨ 재계산 버퍼 (한 번에 한 층치)               S x 149,504B                   222 MiB      842 MiB
#   ⑩ chunked CE 로짓 (256토큰 조각)              256 x 128,256 x 6B             188 MiB      188 MiB
#   -------------------------------------------------------------------------------------------------
#   B 소계                                                                       800 MiB    2,505 MiB
#
# ⑩ 이 문장 길이와 무관하게 "상수"인 것이 loss_type="chunked_nll" 의 핵심입니다.
# 이걸 "nll" 로 두면 ⑩ 이 S 에 정비례해서 아래처럼 불어납니다.
#
#     로짓 bf16 (lm_head 출력)                    S x 128,256 x 2B               381 MiB    1,444 MiB
#     + float32 복사본 (logits.float())           S x 128,256 x 4B               763 MiB    2,888 MiB
#     + log_softmax 저장분 (backward 용)          S x 128,256 x 4B               763 MiB    2,888 MiB
#     합계                                                                     1,907 MiB    7,220 MiB
#
# -----------------------------------------------------------------------------------------------------
# C. 총계
# -----------------------------------------------------------------------------------------------------
#
# A 상주 5,491 MiB + B 가변 + CUDA 컨텍스트/cuBLAS 작업공간 약 600 MiB
#
#   중앙값 길이 샘플 : 약 6.73 GiB
#   최장 샘플        : 약 8.39 GiB
#
# 12GB 카드(WDDM 예약을 빼면 실사용 약 11GB) 기준 여유가 약 2.6 GiB 로, 세 모델 중 가장 빠듯합니다.
# 아래 둘 중 하나만 빠져도 바로 한계를 넘습니다.
#     loss_type="chunked_nll" 을 빼면     최장 샘플에서 +4.7 GiB
#     임베딩 bf16 되돌리기를 빼면          상시 +2.0 GiB
# 즉 이 파일에서 두 최적화는 "있으면 좋은 것"이 아니라 "없으면 안 돌아가는 것"입니다.
#
# -----------------------------------------------------------------------------------------------------
# D. 이 표를 읽을 때 주의할 점
# -----------------------------------------------------------------------------------------------------
...
# 이 모델은 문장이 최대 5,903 토큰으로 1.5배 더 길어 nll 쪽 손해도 그만큼 더 큽니다.
# =====================================================================================================
...
#
#       예전 값 8192는 이 데이터셋의 최대 길이(5,903 토큰)보다 크기 때문에 사실상 자르기가 없었습니다.
#       그 결과 5000 토큰이 넘는 샘플이 그대로 들어와 위 표의 가장 느린 구간에서 학습됩니다.
#
#         이 데이터셋의 실제 토큰 길이 분포 (collate_fn 이 만드는 형식 그대로, 991개 실측)
#           최소 918 / 중앙값 1,559 / 평균 1,631 / 최대 5,903
#           2048 이하 841개 (84.9%)
#           4096 이하 988개 (99.7%)
#
#       !! [중요 정정] 아래 "2048로 낮추라"는 이 권고를 그대로 따르면 안 됩니다.
#          속도만 보고 쓴 권고였는데, 나중에 다시 세어 보니 2048에서는 991개 중 150개(15.1%)가 잘리고
#          그 중 83개는 assistant 헤더까지 잘려 나가 labels가 전부 -100이 됩니다.
#          학습 대상 토큰이 0개인 배치의 cross entropy 는 NaN 이고, NaN 이 한 번 나오면
#          그 스텝에서 모델 가중치가 통째로 망가져 이후 학습이 전부 무의미해집니다.
#          (위 max_seq_length 주석의 표와 collate_fn 안의 "[알아두면 좋은 점 2]" 를 함께 보세요)
#
#          현재 이 파일은 max_seq_length=6144(잘림 0개)를 쓰고, 메모리는 속도가 아니라
#          loss_type="chunked_nll" 로 해결합니다. 2048을 굳이 쓰고 싶다면 학습 전에
#          긴 샘플을 걸러내는 코드를 반드시 함께 넣으십시오.
#
...
#
#       추가 이득: 이 데이터는 길이가 918~5,903 토큰으로 제각각이라, 배치 크기가 2 이상이면
