...
# 메모리 덩어리 크기가 달라집니다. 그러면 PyTorch의 메모리 풀이 잘게 쪼개져서(단편화)
# "총량은 남았는데 연속된 자리가 없는" 상태가 되는데, 이 옵션이 바로 그걸 해결합니다.
#
...
# (맥북 MPS 분기는 양자화를 못 쓰므로 아래 ①② 대신 가중치 전체가 2.6GB(fp16)로 올라갑니다)
#
# -----------------------------------------------------------------------------------------------------
...
#   풀 파인튜닝이었다면 이 두 항목만으로 1.29B x 12바이트 = 약 14 GiB 가 필요했을 겁니다.
...
#
#   ⑩ 이 문장 길이와 무관하게 상수인 것이 loss_type="chunked_nll" 의 핵심입니다.
...
#   gradient_accumulation_steps 를 그만큼 나눠서 실질 배치 크기를 유지하는 선택지가 있습니다.
#   (그 경우 배치 안에서 패딩 낭비가 생기므로 group_by_length=True 를 같이 켜는 게 좋습니다)
#
# -----------------------------------------------------------------------------------------------------
# D. 이 표를 읽을 때 주의할 점
...
#   - ⑨ 에는 어텐션 점수 행렬이 빠져 있습니다. 이건 플랫폼과 마스크 종류에 따라 크게 달라져서,
...
#       torch.cuda.max_memory_reserved()  / 1024**3   # 예약 최대치 (nvidia-smi 와 비슷)
