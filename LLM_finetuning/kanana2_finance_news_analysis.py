...
# 이 파일의 학습 데이터는 샘플마다 길이가 제각각(665~3,951 토큰)이라서 매 스텝 요구하는
# 메모리 덩어리 크기가 달라지고, 그러면 PyTorch의 메모리 풀이 잘게 쪼개져(단편화)
# "총량은 남았는데 연속된 자리가 없는" 상태가 됩니다.
#
# !! [정정] 예전에는 여기에 expandable_segments:True 한 줄만 있었는데, 이 옵션은
#           Windows에서 아무 일도 하지 않는다는 것이 확인되어 아래처럼 나눴습니다.
#
#   [expandable_segments:True]  메모리 덩어리를 "새로 사오는" 대신 "늘려 쓰게" 해서 단편화를 없앱니다.
#                               단, Linux 전용입니다. PyTorch 소스(c10/cuda/CUDAAllocatorConfig.h)에서
#                               PYTORCH_C10_DRIVER_API_SUPPORTED 매크로가 없으면 무조건 false를 돌려주고
#                               "expandable_segments not supported on this platform" 경고만 남깁니다.
#                               이 매크로는 Linux 빌드에서만 켜집니다.
#
#   [garbage_collection_threshold:0.8]  이건 플랫폼과 무관하게 동작합니다.
#                               "PyTorch가 쥐고 있는 캐시가 GPU 용량의 80%를 넘어가면,
#                                지금 안 쓰는 덩어리는 알아서 반납하라"는 뜻입니다.
#                               Windows에서 메모리가 계단식으로 불어나는 것을 막아주는 핵심 옵션입니다.
#
# 자세한 배경은 아래 max_seq_length 위의 [GPU 메모리 문제 총정리] / [추가 조사] 주석을 보세요.
# torch를 import 하기 전에 설정해야 적용되며, CUDA가 없는 맥북에서는 아무 일도 하지 않습니다.
# (setdefault라서 터미널에서 이미 지정했다면 그 값을 존중합니다)
# =====================================================================================================
import os
import sys

if sys.platform == "win32":
    # Windows에서는 expandable_segments가 무시되므로 넣지 않습니다 (넣으면 경고만 뜹니다)
...
if torch.cuda.is_available():
    ...
    ##
    ##   embed_tokens (= lm_head, 이 모델은 두 층이 가중치를 공유) 164,167,680개
    ##     bf16(2바이트)   ->   313 MB
    ##     float32(4바이트) ->  626 MB   <- 그냥 두면 이쪽
    ##
    ## 313MB 차이도 아깝지만 진짜 문제는 따로 있습니다.
    ## 임베딩이 float32면 그 출력인 은닉 상태(hidden states)도 float32가 되고,
    ## 잔차 연결(residual)을 타고 32개 층 내내 float32로 유지되어 활성화 메모리가 통째로 2배가 됩니다.
    ##
    ## 그래서 "2차원 이상인 파라미터(= 임베딩처럼 큰 행렬)"만 bf16으로 되돌립니다.
    ## 1차원 파라미터(RMSNorm 등)는 float32로 남겨두는데, 이건 QLoRA 논문이 권장하는 방식입니다.
    ## 크기가 작아서(전부 합쳐 8만개) 메모리 이득이 없는 반면, float32로 둬야 학습이 안정적이기 때문입니다.
    ##
...
# =====================================================================================================
# [GPU 메모리 문제 총정리] "1.3B 모델을 QLoRA로 돌리는데 왜 12GB GPU가 터지는가"
#
# RTX 3080 Ti(VRAM 12GB) + 시스템 RAM 32GB / Windows 에서 이 스크립트를 돌렸을 때 실제로 겪은 증상입니다.
#   - 학습 초반     : GPU 메모리를 12GB 가까이 점유하며 그럭저럭 돌아감
#   - 학습 중반부터 : GPU 메모리가 넘쳐 시스템 RAM 까지 31GB 이상 잡아먹고 속도가 급락
#
# "파라미터가 1.29B밖에 안 되고 QLoRA로 4비트까지 줄였는데 왜?" 가 자연스러운 의문입니다.
# 결론부터 말하면, 범인은 모델 가중치가 아니라 "로짓(logits)" 이라는 중간 결과물입니다.
#
# -----------------------------------------------------------------------------------------------------
# 1. 로짓이 뭐길래 — 이 모델에서는 가중치보다 10배 무겁습니다
# -----------------------------------------------------------------------------------------------------
# 언어 모델은 토큰 하나하나마다 "다음에 올 단어는 사전에 있는 128,256개 중 무엇일까"를 점수로 뱉습니다.
# 이 점수표가 로짓이고, 크기는 (문장의 토큰 수) x (사전 크기) 입니다.
#
#   문장 하나가 3,951 토큰이고 사전이 128,256개라면
#     3,951 x 128,256 = 약 5억 개의 숫자
#   손실 계산은 정밀도를 위해 float32(숫자 1개당 4바이트)로 하므로   ->   1.89 GB
#
...
#   => 1.89GB x 3 = 약 5.7 GB
#
# 반면 이 모델의 가중치는 QLoRA로 4비트 압축하면 겨우 538MB 입니다.
# 즉 "중간 결과물이 모델 본체보다 10배 무거운" 상황이고, QLoRA는 여기에 아무 도움이 되지 않습니다.
# QLoRA가 줄여주는 것은 오직 "가중치"뿐이며, 활성화(activation)와 로짓은 1비트도 줄여주지 못하기 때문입니다.
#
#   이 모델의 실제 메모리 구성 (문장 길이 3,951 기준)
#     4비트로 압축된 본체 (1,129M 파라미터)                538 MB
#     embed_tokens = lm_head (164M, 두 층이 가중치 공유)    313 MB (bf16) / 626 MB (float32)
#     LoRA 1,966,080개 + AdamW 옵티마이저 상태               22 MB
#     로짓 3벌                                           5,660 MB   <- 압도적 1위
#
# [초보자가 빠지기 쉬운 함정]
#   "파라미터 수"만 보고 메모리를 예측하면 반드시 틀립니다. 메모리를 결정하는 것은
#     (a) 파라미터 수  (b) 사전 크기(vocab_size)  (c) 문장 길이  (d) 배치 크기
#   네 가지이고, QLoRA는 이 중 (a) 하나만 줄여줍니다.
#   특히 한국어 모델은 한글을 담느라 사전이 12만~17만개로 큰 편이라 (b)의 영향이 매우 큽니다.
#
# -----------------------------------------------------------------------------------------------------
# 2. "초반엔 괜찮다가 중반에 터지는" 이유
# -----------------------------------------------------------------------------------------------------
# 로짓 크기는 문장 길이에 정비례하는데, 이 데이터셋의 문장 길이는 제각각입니다.
# (아래 max_seq_length 주석의 실측 분포 참고: 최소 665 ~ 최대 3,951 토큰)
#
# 학습은 데이터를 무작위 순서로 도는데, 운 좋게 짧은 문장이 이어지면 2~3GB로 조용히 돌다가
# 긴 문장을 만나는 순간 7GB로 튑니다. 즉 "중반부터 터진다"가 아니라 "긴 문장을 만나면 터진다"가 정확합니다.
#
# 여기에 "메모리 단편화"가 겹칩니다.
# 1.89GB짜리 로짓은 반드시 "연속된" 메모리 덩어리여야 하는데, 매 스텝 요구 크기가 달라지다 보니
# PyTorch의 메모리 풀이 잘게 쪼개져서 "총량은 남았는데 연속된 자리가 없는" 상태가 됩니다.
# 이게 학습이 진행될수록 심해지므로, 시간이 지날수록 점유량이 계단식으로 늘어나는 것처럼 보입니다.
...
#            메모리 풀을 고정 크기 조각이 아니라 늘였다 줄였다 할 수 있는 형태로 바꿔 단편화를 크게 줄입니다.
#
# -----------------------------------------------------------------------------------------------------
...
#   [2] gradient_checkpointing 을 항상 True 로 고정     -> O(길이²) 폭발 방지 (위 SFTConfig 안)
#   [3] max_seq_length 를 8192 -> 4096                 -> 최악의 경우 상한을 못박음 (바로 아래)
#   [4] QLoRA 분기에서 임베딩을 다시 bf16 으로 되돌림    -> 활성화 메모리 절반 (위 CUDA 분기 안)
#   [+] 파일 맨 위 PYTORCH_CUDA_ALLOC_CONF 설정        -> 단편화 완화
#
#   네 가지를 모두 적용하면 문장 길이 3,951 기준 최대 점유가 약 7GB -> 2GB 수준으로 내려가,
#   12GB GPU에서 여유 있게 돌아갑니다.
#
# -----------------------------------------------------------------------------------------------------
# 5. 내 환경에서 직접 확인하는 방법
# -----------------------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------------------
# =====================================================================================================
# [추가 조사] "학습 중간에 갑자기 메모리가 치솟는" 현상의 정체
#
# 바로 아래 [GPU 메모리 문제 총정리]는 "한 스텝이 메모리를 얼마나 쓰는가"에 대한 이야기입니다.
# 여기서 다루는 것은 그것과 완전히 별개인 문제로, 실제 RTX 3080 Ti(12GB)에서 관찰된 증상입니다.
#
#   - 학습 시작 직후   : GPU 5GB 정도로 안정
#   - 조금 뒤          : 9GB로 한 계단 올라가서 유지
#   - 158 step 부근    : 갑자기 16GB (공유 메모리까지 끌어다 씀)
#   - 168 step 부근    : 또 갑자기 24.3GB. PC 일반 RAM 점유도 31GB를 넘김
#
#   한 번 올라간 사용량이 절대 내려오지 않는 "계단" 모양인 것이 핵심 단서입니다.
#
# -----------------------------------------------------------------------------------------------------
# 1. 158 / 168 step 은 우연이 아닙니다 — "문장 길이 신기록"이 나오는 지점입니다
# -----------------------------------------------------------------------------------------------------
#
# Trainer가 데이터를 섞는 순서는 seed=42로 고정되어 있어서 학습을 돌리지 않고도 재현할 수 있습니다.
# 훈련 데이터 793개를 이 모델(kanana-2-1.3b-instruct)의 토크나이저로 재보고,
# 각 step에 들어가는 가장 긴 문장을 추적한 결과입니다.
#
#   step       그 step의 최대 길이    지금까지의 최고 기록
#   ------------------------------------------------------
#   1                        1,176                   1,176  <- 신기록
#   2                        1,445                   1,445  <- 신기록
#   4                        1,816                   1,816  <- 신기록
#   10                       3,094                   3,094  <- 신기록
#   159                      3,817                   3,817  <- 신기록   (진행표에는 158로 보임)
#   169                      3,951                   3,951  <- 신기록   (진행표에는 168로 보임)
#
# 관찰된 158 / 168과 정확히 일치합니다. 1 차이가 나는 이유는 진행표(진행 막대)가
# "끝난 step 수"를 세기 때문입니다. 159번째 step이 계산되는 동안 화면에는 158이 찍혀 있습니다.
#
# -----------------------------------------------------------------------------------------------------
# 2. 범인은 훈련 데이터 793개 중 딱 3개입니다
# -----------------------------------------------------------------------------------------------------
#
#   길이 분포 : min 665 / 중앙값 1,088 / 평균 1,145 / p90 1,541 / p99 2,174 / 최대 3,951
#   상위 12개 : 2,087 2,128 2,157 2,174 2,176 2,260 2,345 2,357 2,589 │ 3,094 3,817 3,951
#                                                                      └ 여기서 뚝 끊깁니다
#
#   길이 초과 기준       샘플 수      비율
#   --------------------------------------
...
#   2,048 초과              14개      1.8%
#   2,560 초과               4개      0.5%
#   3,072 초과               3개      0.4%   <- 이 3개가 문제의 전부
#
#   중앙값과 p99는 평범한데 그 위로 단 3개가 튀어 있습니다. 그리고 이 3개는
#   같은 폴더의 다른 두 모델에서도 똑같이 범인입니다. 토크나이저가 달라 토큰 수는 다르지만
#   원본 뉴스 기사가 같기 때문에, 세 모델 모두 고수위 갱신 step이 1, 2, 4, 10, 159, 169로 동일합니다.
#
# -----------------------------------------------------------------------------------------------------
# 3. 그 메모리는 "쓰이는" 게 아니라 "잡혀만" 있습니다
# -----------------------------------------------------------------------------------------------------
#
# 위와 같은 길이 순서로 실제 모델을 돌려서 세 가지를 따로 쟀습니다.
# (kanana-2로 맥북 MPS에서 측정. fp16 비양자화라 절대값은 CUDA와 다르지만 '모양'은 동일합니다)
#
#   문장 길이       실제 살아있는 양(최대)      스텝 끝난 뒤 남은 양     드라이버가 잡고 있는 양
#   --------------------------------------------------------------------------------------------
#   1,176                          3.62 GB                   2.63 GB                     7.75 GB
#   1,445                          3.94 GB                   2.63 GB                    10.35 GB
#   1,816                          4.21 GB                   2.63 GB                    13.15 GB
#   3,094                          5.17 GB                   2.63 GB                    21.06 GB   <- 관찰된 9GB 계단
#   3,817                          6.20 GB                   2.63 GB                    30.96 GB   <- 관찰된 158 step, 16GB
#   3,951                          6.11 GB                   2.63 GB                    40.54 GB   <- 관찰된 168 step, 24.3GB
#
# 가운데 열이 매 스텝 2.63GB로 완전히 똑같습니다. 메모리 누수도 없고 해제도 정상이라는 뜻입니다.
# 실제로 필요한 양(왼쪽 열)도 3.6 -> 6.2GB로 완만하게 오를 뿐입니다.
# 그런데 오른쪽 열만 7.75 -> 40.54GB로 폭주합니다.
# nvidia-smi와 Windows 작업 관리자가 보여주는 값이 바로 이 오른쪽 열입니다.
#
# [왜 이런 일이 생기나]
# PyTorch는 GPU 메모리를 OS에 곧바로 돌려주지 않습니다. GPU 메모리를 빌리고 반납하는 작업
# (cudaMalloc / cudaFree)이 매우 느리기 때문에, 한 번 받아온 메모리를 자기 "캐시"에 쌓아두고
# 재활용합니다. 이것을 캐싱 할당자(caching allocator)라고 합니다.
#
# 문제는 캐시에 든 덩어리를 "요청한 크기보다 크거나 같을 때만" 재사용할 수 있다는 점입니다.
# 창고에 비유하면 이렇습니다.
#
#   창고에 1,176짜리 상자들을 쌓아뒀는데, 이번엔 1,445짜리 물건이 들어왔습니다.
#   기존 상자에는 안 들어갑니다. 그래서 새 상자를 사 옵니다.
#   그런데 낡은 1,176 상자는 버리지도 않습니다. 창고에 그대로 쌓여 있습니다.
#   길이가 계속 신기록을 세우면, 창고에는 '쓸 수 없는 상자'만 계속 쌓여 갑니다.
#
# 그래서 드라이버가 잡고 있는 양은 "지금 필요한 최대치"가 아니라
# "지금까지 본 모든 크기의 합"에 가까워집니다. 이것이 계단이 내려오지 않는 이유입니다.
#
# -----------------------------------------------------------------------------------------------------
# 4. Windows에서 유독 심해지는 이유 — 안전장치가 꺼져 있습니다
# -----------------------------------------------------------------------------------------------------
#
# 사실 PyTorch에는 이 상황을 막는 안전장치가 있습니다.
#   "GPU 메모리 요청이 실패하면 -> 캐시에 쌓인 상자를 전부 버리고 -> 다시 요청한다"
# 이 경로만 돌면 159 step에서 낡은 상자들이 정리되고 메모리가 제자리를 찾습니다.
#
# 그런데 NVIDIA 드라이버(R535 이상)의 "CUDA 시스템 메모리 폴백"이 기본으로 켜져 있으면,
# VRAM이 모자라도 요청이 실패하지 않습니다. 대신 조용히 PC의 일반 RAM에 자리를 잡아줍니다.
#
#   -> PyTorch는 실패를 통보받지 못하므로, 위 안전장치가 영원히 작동하지 않습니다.
#   -> 캐시는 24.3GB까지 계속 불어납니다. (일반 RAM을 31GB 점유한 것의 정체가 이것입니다)
#   -> 게다가 일반 RAM에 놓인 데이터는 접근할 때마다 PCIe를 왕복하므로 학습이 극단적으로 느려집니다.
#      즉 이 폴백은 '살려주는 기능'처럼 보이지만 실제로는 학습을 망가뜨립니다.
#
# [끄는 방법]  NVIDIA 제어판 -> 3D 설정 관리 -> 프로그램 설정 탭
#              -> python.exe 추가 -> "CUDA - 시스템 메모리 폴백 정책"
#              -> "시스템 메모리 폴백 없음 선호"(Prefer No System Memory Fallback)
#
# 이렇게 하면 메모리가 정말 모자랄 때 깔끔하게 OOM 에러가 납니다. 그리고 그 전에
# 위 안전장치가 먼저 돌기 때문에, 대부분의 경우 OOM 없이 그냥 정상 동작하게 됩니다.
# 코드 수정 없이 설정만으로 끝나면서 효과가 가장 큰 대책입니다.
#
# -----------------------------------------------------------------------------------------------------
# 5. 대책과 실측 효과
# -----------------------------------------------------------------------------------------------------
#
# kanana-2 로 실제 170 step 을 돌려 가며 청소 전략만 바꿔 재봤습니다.
# (맥북 MPS / fp16 비양자화라 절대값은 CUDA 보다 훨씬 큽니다. 전략 사이의 '비교'만 보세요)
#
#   청소 전략                              169 step 시점        최종   청소 횟수
#   ----------------------------------------------------------------------------
#   아무것도 안 함                                     —     40.5 GB           0
#   25 step 마다                                 38.6 GB     35.6 GB           6
#   길이 신기록일 때 + 25 step 마다              32.7 GB     31.0 GB          12
#   캐시가 용량의 35% 를 넘을 때만               21.9 GB     10.2 GB          33   <- 채택
#   매 배치마다                                        —      7.0 GB         170
#
# [처음에 틀렸던 생각] "길이 신기록이 나오는 step 에서만 비우면 된다"고 보고 그렇게 만들었습니다.
#   신기록 step 에서 이전 크기의 덩어리가 전부 쓸모없어지는 것은 사실이니까요. 그런데 아니었습니다.
#
#     step 100 (len 1,152)   reserved   4.0 GB   <- 150 step 에서 청소한 직후 값
#     step 158 (len 2,048)   reserved  16.6 GB   <- 신기록이 하나도 없는데 8 step 만에 16 GB
#
#   신기록이 없는 구간에서도 쌓입니다. 진짜 원인은 "서로 다른 텐서 모양의 개수" 였습니다.
#   128 배수로 버킷팅하면 모양이 18 종류인데, 모양마다 활성값 덩어리 한 벌이 캐시에 따로 남습니다.
#   18 x 약 2GB = 약 36GB 로, 위 표의 천장 40.5GB 와 맞아떨어집니다.
#
# [그래서 올바른 조건은] "N step 마다"도 "신기록일 때"도 아니라 "캐시가 실제로 커졌을 때" 입니다.
#   메모리가 여유로우면 한 번도 청소하지 않고, 천장에 닿을 때만 청소합니다.
#   위 표에서 유일하게 "계속 늘어나지 않고 일정 수준에 머무는" 전략입니다.
#
# [!! 측정 환경의 한계 !!]
#   위 숫자는 전부 맥북 MPS 실측입니다. 문제가 난 환경은 CUDA + Windows 이고 그쪽은 4bit
#   양자화까지 걸려 있어 절대값이 훨씬 작습니다 (아래 [trainer.train() 메모리 예상치] 참고).
#   전략 사이의 우열은 옮겨 가지만, GB 수치를 그대로 기대하면 안 됩니다.
#   그리고 코드로 하는 이 모든 대책보다 위 4번의 '시스템 메모리 폴백 끄기'가 여전히 1순위입니다.
#   폴백을 끄면 PyTorch 자신의 캐시 반납 경로가 되살아나기 때문입니다.
#
# [버킷팅] 문장 길이를 일정 배수로 올려 패딩하면 서로 다른 텐서 모양의 개수가 줄어듭니다.
#   모양이 적을수록 캐시에 쌓이는 덩어리 종류도 줄지만, 패딩만큼 계산이 낭비됩니다.
#
#   버킷 크기           서로 다른 길이    패딩으로 버리는 계산량
#   ------------------------------------------------------------
#   (버킷팅 안 함)             551종류                      0.0%
#   128 배수                    18종류                      5.6%   <- 채택
#   256 배수                    12종류                     10.9%
#   512 배수                     7종류                     22.5%
#
#   위 스위치 2가 이미 상한을 잡아 주므로, 여기서는 계산 낭비가 가장 적은 128 을 골랐습니다.
#   메모리가 더 빠듯하면 256 이나 512 로 올리세요. (짧은 6 step 테스트에서 512 배수만
#   켰을 때 40.5GB -> 28.7GB 였습니다. 대신 계산이 22% 늘어납니다)
#
# [가장 단순한 길] 아래 DROP_SAMPLES_LONGER_THAN 을 켜면 유난히 긴 샘플 몇 개를 아예 빼버립니다.
#   이 모델 기준 3072 토큰 초과는 3개뿐이고, 빼고 나면 최대 길이가 2,589 토큰으로 떨어져
#   위 계단 두 개가 통째로 사라집니다. 학습 데이터 0.4%를 버리는 선택이라 기본값은 꺼둠(None)입니다.
# =====================================================================================================
# [max_seq_length] 한 샘플이 가질 수 있는 최대 토큰 수 (넘으면 뒤에서부터 잘라냄)
#
# 이 값이 곧 "로짓 크기의 상한"이라 메모리를 좌우하는 가장 직관적인 손잡이입니다.
#
# 이 토크나이저로 데이터 991개를 전부 실측한 분포:
#   최소 665 / 중앙값 1,083 / 평균 1,134 / 최대 3,951 토큰
#   (p90 = 1,517  p95 = 1,718  p99 = 2,157)
#
#   max_seq_length   잘리는 샘플     정답까지 통째로 사라지는 샘플   최대 로짓 1벌
#   --------------------------------------------------------------------------------
#        2048         14개 (1.4%)             7개                    0.98 GB
#        3072          3개 (0.3%)             2개                    1.47 GB
#        4096          0개 (0.0%)             0개                    1.89 GB  <- 이 파일이 쓰는 값
#        8192          0개 (0.0%)             0개                    1.89 GB  <- 예전 값
#
# 예전 값 8192는 실제 최대 길이(3,951)의 두 배가 넘어서, 사실상 아무 일도 하지 않는 값이었습니다.
# 4096으로 낮추면 잘리는 샘플은 여전히 0개인데 "최악의 경우 얼마나 쓸지"의 상한이 확실히 못박힙니다.
#
...
#        Llama-VARCO 토크나이저 기준 숫자였습니다. kanana-2 는 한국어 토크나이저 효율이 좋아서
#        같은 데이터가 30% 가량 짧게 인코딩됩니다. 토큰 길이는 반드시 "그 모델의 토크나이저"로 재야 합니다.
# -----------------------------------------------------------------------------------------------------
max_seq_length = 4096

# =====================================================================================================
# [메모리 대책] GPU 메모리가 계단식으로 불어나는 것을 막는 스위치 2개
#
# 왜 필요한지는 바로 위 [추가 조사] 주석에 자세히 적어 두었습니다. 한 줄 요약은 이렇습니다.
#   "PyTorch 는 한 번 받아온 GPU 메모리를 OS 에 돌려주지 않고 캐시에 쌓아두는데,
#    배치 모양이 바뀔 때마다 재사용 못 하는 덩어리가 늘어나면서 캐시만 계속 불어난다"
# =====================================================================================================
import gc
...
# [스위치 1] 패딩 길이를 이 값의 배수로 올림 (None 이면 끔)
## 배치마다 가장 긴 문장에 딱 맞춰 패딩하면 텐서 모양이 551종류나 생기고, 모양마다 활성값 덩어리
## 한 벌이 캐시에 따로 남습니다. 128 의 배수로 맞추면 모양이 18종류로 줄고 계산 낭비는 5.6%뿐입니다.
## 메모리가 정말 빠듯하면 256 이나 512 로 올려 모양 수를 더 줄일 수 있습니다.
## (배수별 비교는 위 [추가 조사] 5번의 표 참고)
PAD_TO_MULTIPLE_OF = 128

# [스위치 2] 캐시가 GPU 용량의 이 비율을 넘으면 반납 (0 이나 None 이면 끔)
## 청소 조건을 "N step 마다"나 "길이 신기록일 때"로 잡으면 실패합니다. 실측 근거는 [추가 조사] 5번에
## 표로 정리해 두었습니다. 조건은 반드시 "캐시가 실제로 커졌을 때"여야 상한이 잡힙니다.
##
## 참고: CUDA 에서는 이 파일 맨 위의 garbage_collection_threshold:0.8 이 똑같은 일을 PyTorch
##       할당자 내부에서 더 효율적으로 해 줍니다. 아래 스위치는 그 기능이 없는 환경(MPS 등)을 위한
##       이식성 있는 대체품이자, CUDA 에서는 이중 안전장치입니다.
## 이 검사는 collate_fn 안에서 매 배치마다 돌지만, 숫자 비교 한 번이라 비용이 없습니다.
...
args = SFTConfig(
    ...
    ##
    ## [메모리 절약 (2/3)] !! 이 모델에서는 플랫폼과 무관하게 반드시 True 여야 합니다 !!
    ##
    ## 한때 이 줄은 gradient_checkpointing=torch.cuda.is_available() 이었습니다.
    ## "1.3B라 가중치가 2.6GB뿐이고 MPS 실측이 3.36GB니 맥북에서는 꺼서 속도를 벌자"는 판단이었고,
    ## 6층짜리 작은 테스트 모델로 재보니 켜면 3.89s / 끄면 2.26s 라 실제로 1.7배 빨랐습니다.
    ##
    ## 그런데 진짜 kanana-2-1.3b 로 다시 재보니 결과가 정반대였습니다.
    ## (batch=1, forward 종료 시점에 남아 있는 메모리)
    ##
    ##     문장 길이    체크포인팅 ON      체크포인팅 OFF
    ##     ---------------------------------------------
    ##      1,024          0.25 GB           11.0 GB
    ##      2,048          0.50 GB           43.0 GB
    ##      3,951          0.72 GB           측정 불가 (OOM)
    ##
    ## 길이가 2배가 되는데 메모리는 4배가 되는 O(길이²) 패턴입니다. 앞의 3.36GB 는
    ## "한 스텝이 끝난 뒤"를 잰 값이라 forward 도중의 봉우리를 놓친 숫자였습니다.
    ##
    ## [왜 이 모델만 이렇게 심한가]
    ## kanana-2 는 32개 층 중 24개가 sliding window attention(창 크기 1024)입니다.
    ## 문장이 1024 토큰을 넘으면 transformers 가 (1, 1, 길이, 길이) 짜리 마스크 행렬을 실제로 만들어야 하고,
    ## 명시적인 마스크가 들어오면 PyTorch 는 가장 빠르고 메모리를 안 쓰는 FlashAttention 커널을 쓸 수 없습니다.
    ## FlashAttention 이 배제된 뒤 어디로 떨어지는지는 플랫폼마다 다릅니다.
    ##   맥북(MPS) : 마스크를 받아주는 대체 커널이 없어 math 백엔드로 떨어지고,
    ##               층마다 (1, 32, 길이, 길이) 크기의 점수 행렬이 통째로 만들어집니다.
    ##               위 표의 11GB / 43GB 가 바로 이 경우를 실측한 값입니다.
    ##   CUDA      : 마스크를 지원하는 memory-efficient 백엔드가 있어서 점수 행렬을 통째로
    ##               만들지는 않습니다. 위 표만큼 극단적이지는 않지만, FlashAttention 보다
    ##               느리고 메모리도 더 쓰는 것은 마찬가지입니다.
    ## (직접 확인: 나머지 8개 full attention 층은 마스크가 None 이라 빠른 경로를 탑니다.
    ##  같은 폴더의 Konan-LLM-OND / Llama-VARCO 는 sliding window 가 없어서 전 층이 None 입니다)
    ##
    ## 체크포인팅을 켜면 중간 결과를 저장하지 않고 backward 때 한 층씩 다시 계산하므로,
    ## 이 거대한 점수 행렬이 "한 번에 한 층치"만 존재하게 되어 폭발이 일어나지 않습니다.
    ##
    ## 재계산 때문에 20~30% 느려지는 건 사실이지만, 43GB를 쓰다 스왑으로 떨어지는 것보다는 압도적으로 낫습니다.
    ...
    ## "로짓 3벌 = 5.7GB" 문제를 정면으로 해결하는 옵션입니다.
    ##
    ...
    ##
    ## 참고) 이 데이터셋의 실제 토큰 길이 분포 (kanana-2 토크나이저로 챗 템플릿 적용 후, 991개 실측)
    ##       최소 665 / 중앙값 1,083 / 평균 1,134 / 최대 3,951
    ##       2048 이하 977개 (98.6%)
    ##       3072 이하 988개 (99.7%)
    ##       4096 이하 991개 (100%)   <- 현재 max_seq_length=4096은 전부 온전히 담음
    ##       (길이를 늘리면 로짓 크기가 정비례해서 커지므로, 메모리 이야기는 max_seq_length 주석을 보세요)
...
def tokenize_with_assistant_labels(messages):
    ...
    ##
    ## kanana-2 토크나이저로 실측한 이 데이터셋의 길이 분포는 중앙값 1,083 / 평균 1,134 / 최대 3,951 이고
    ## 현재 max_seq_length=4096 이므로 잘리는 샘플이 0개라서 지금은 이 함정에 걸리지 않습니다.
    ## 하지만 메모리를 더 아끼려고 값을 낮추는 순간 바로 위험해집니다. (max_seq_length 주석의 표 참고)
    ##   2048로 낮추면 14개가 잘리고 그 중 7개는 정답이 통째로 사라집니다.
...
def collate_fn(batch):
    ...
    ## [메모리 대책] 패딩 길이를 PAD_TO_MULTIPLE_OF 의 배수로 올림 (위 [추가 조사] 5번 참고)
    ...
    ## [메모리 대책 스위치 2] 이번 배치를 계산하기 전에, 캐시가 너무 커졌으면 먼저 반납합니다.
...
# =====================================================================================================
# [선택] 유난히 긴 소수의 샘플을 학습에서 제외
#
# 위 [추가 조사] 2번에서 확인했듯, 메모리 계단을 만드는 범인은 훈련 데이터 793개 중 단 3개입니다.
# 아래 값을 3072로 바꾸면 그 3개를 빼고 학습합니다.
#   - 최대 길이가 4096 -> 2,589 토큰으로 떨어져 159 / 169 step의 계단이 통째로 사라집니다.
#   - 대신 학습 데이터의 0.4%를 버리게 됩니다.
#
# 데이터를 버리는 선택이라 기본값은 None(끔)입니다. 위 두 대책(캐시 반납 + 버킷팅)으로도
# 메모리가 부족할 때 마지막 수단으로 켜세요.
# =====================================================================================================
DROP_SAMPLES_LONGER_THAN = None  # 예: 3072

if DROP_SAMPLES_LONGER_THAN is not None:
    ## 길이를 재는 방법: collate_fn 에 샘플 1개짜리 배치를 넣어 보고 attention_mask 의 합을 셉니다.
    ## attention_mask 는 실제 토큰이면 1, 패딩이면 0 이므로 그 합이 곧 패딩을 뺀 진짜 길이입니다.
    ## (학습에 들어가는 것과 똑같은 경로로 재는 것이라 계산이 어긋날 일이 없습니다)
    _before = len(train_dataset)
    _keep = [
        i
        for i, _m in enumerate(train_dataset["messages"])
        if int(collate_fn([{"messages": _m}])["attention_mask"][0].sum()) <= DROP_SAMPLES_LONGER_THAN
    ]
    train_dataset = train_dataset.select(_keep)
    print(f"[길이 필터] {DROP_SAMPLES_LONGER_THAN} 토큰 초과 {_before - len(train_dataset)}개 제외 "
          f"-> 훈련 데이터 {_before}개에서 {len(train_dataset)}개로 줄었습니다")
...
#   항목                                          계산식                            크기
#   ------------------------------------------------------------------------------------
#   ① 4bit 양자화된 본체 가중치                   1,127M x 0.5B                  538 MiB
#   ② 양자화 상수 (double quant)                  1,127M x 0.127bit               17 MiB
#   ③ 임베딩 = lm_head (bf16, 가중치 공유)        164M x 2B                      313 MiB
#   ④ LayerNorm 등 1차원 파라미터 (fp32 유지)     83K x 4B                       0.3 MiB
#   ⑤ LoRA 가중치 (fp32)  <- 유일한 학습 대상     1,966,080 x 4B                 7.5 MiB
#   ⑥ LoRA 그레디언트 (fp32)                      1,966,080 x 4B                 7.5 MiB
#   ⑦ AdamW 옵티마이저 상태 m, v (fp32)           1,966,080 x 8B                15.0 MiB
#   ------------------------------------------------------------------------------------
#   A 소계                                                                       898 MiB
#
# [여기가 가장 놀라운 지점입니다]
# 보통 학습 메모리의 주범으로 꼽히는 ⑥그레디언트 + ⑦옵티마이저 상태가 합쳐서 겨우 22 MiB 입니다.
# LoRA 가 전체 파라미터의 0.152% 만 학습하기 때문입니다.
# 풀 파인튜닝이었다면 이 두 항목만으로 1.29B x 12바이트 = 약 14.4 GiB 가 필요했을 겁니다.
#   (그레디언트 fp32 4B + Adam 의 1차 모멘트 m 4B + 2차 모멘트 v 4B = 파라미터당 12바이트)
# "QLoRA 는 가중치를 4비트로 줄이는 기법"이라고들 하지만, 실제로 가장 크게 아끼는 것은 ①이 아니라 ⑥⑦입니다.
#
# ③을 fp32 로 두면(= 위 CUDA 분기에서 bf16 으로 되돌리지 않으면) 626 MiB 가 되어 313 MiB 손해입니다.
#
# -----------------------------------------------------------------------------------------------------
# B. 스텝마다 변하는 메모리 — 문장 길이 S 에 비례
# -----------------------------------------------------------------------------------------------------
#
# 이 데이터셋의 실측 길이: 최소 665 / 중앙값 1,083 / 최대 3,951 토큰
#
#   항목                                          계산식                         S=1,083      S=3,951
#   -------------------------------------------------------------------------------------------------
#   ⑧ 체크포인트 저장분 (32개 층의 입력)          S x 81,920B                     85 MiB      309 MiB
#   ⑨ 재계산 버퍼 (한 번에 한 층치)               S x 75,520B                     78 MiB      285 MiB
#   ⑩ chunked CE 로짓 (256토큰 조각)              256 x 128,256 x 6B             188 MiB      188 MiB
#   -------------------------------------------------------------------------------------------------
#   B 소계                                                                       350 MiB      781 MiB
#
# ⑩ 이 문장 길이와 무관하게 "상수"인 것이 loss_type="chunked_nll" 의 핵심입니다.
# 이걸 "nll" 로 두면 ⑩ 이 S 에 정비례해서 아래처럼 불어납니다.
#
#     로짓 bf16 (lm_head 출력)                    S x 128,256 x 2B               265 MiB      967 MiB
#     + float32 복사본 (logits.float())           S x 128,256 x 4B               530 MiB    1,933 MiB
#     + log_softmax 저장분 (backward 용)          S x 128,256 x 4B               530 MiB    1,933 MiB
#     합계                                                                     1,325 MiB    4,833 MiB
#
# -----------------------------------------------------------------------------------------------------
# C. 총계
# -----------------------------------------------------------------------------------------------------
#
# A 상주 898 MiB + B 가변 + CUDA 컨텍스트/cuBLAS 작업공간 약 600 MiB
#
#   중앙값 길이 샘플 : 약 1.81 GiB
#   최장 샘플        : 약 2.23 GiB
#
# 12GB 카드(WDDM 예약을 빼면 실사용 약 11GB) 기준 여유가 8.8 GiB 나 됩니다.
# 세 모델 중 가장 넉넉하므로, 속도를 원하면 per_device_train_batch_size 를 2~4로 올리고
# gradient_accumulation_steps 를 그만큼 나눠 실질 배치 크기를 유지하는 선택지가 있습니다.
# (그 경우 배치 안에서 패딩 낭비가 생기므로 group_by_length=True 를 같이 켜는 게 좋습니다)
#
# -----------------------------------------------------------------------------------------------------
# D. 이 표를 읽을 때 주의할 점
...
#   둘의 차이가 크게 벌어지는 원인과 대책은 위 [추가 조사] 주석에 따로 정리해 두었습니다.
#   (요약: 문장 길이가 신기록을 세울 때마다 재사용 못 하는 덩어리가 캐시에 쌓입니다)
# - 위 B 의 S 에는 패딩이 포함됩니다. PAD_TO_MULTIPLE_OF=128 때문에 실제 S 는 128 의 배수로
#   올림되므로, 표의 값보다 최대 127 토큰치만큼 커질 수 있습니다 (평균 5% 안팎).
#
# -----------------------------------------------------------------------------------------------------
...
#
# 단 이 실측은 맥북(MPS)에서 잰 값이라 어텐션 점수 행렬의 순간 봉우리가 CUDA와 다릅니다.
# kanana-2 는 sliding window 층 때문에 MPS에서 층당 최대 1GB 수준의 봉우리가 추가로 생깁니다.
# (위 gradient_checkpointing 주석 참고. CUDA 에서는 이 정도로 심하지 않습니다)
# =====================================================================================================
