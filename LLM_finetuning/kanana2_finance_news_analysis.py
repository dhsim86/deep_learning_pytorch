...
# -----------------------------------------------------------------------------------------------------
#
#   대책                               드라이버 점유량   이 파일 적용 여부
#   ------------------------------------------------------------------------------------
#   (아무 대책 없음)                           40.5 GB   기준값
#   empty_cache() 를 매 스텝 호출               7.0 GB   O  (실제로는 25 step마다 — 아래 EmptyCacheCallback)
#   길이를 512 배수로 버킷팅                   28.7 GB   O  (실제로는 128 배수 — 아래 PAD_TO_MULTIPLE_OF)
#   시스템 메모리 폴백 끄기                (측정 불가)   제어판 설정이라 코드 밖
#
# [empty_cache()] "창고에서 안 쓰는 상자를 전부 버려라"는 명령입니다. 효과가 압도적입니다.
#   다만 버린 메모리를 나중에 다시 받아오는 데 시간이 걸리므로 매 스텝 부르면 느려집니다.
#   그래서 25 step마다 부르도록 했습니다. 실측해 보면 신기록 직후에는 잠깐 튀었다가
#   (step 10에서 23.4GB) 다음 청소 때 회수됩니다 (step 50에서 4.1GB).
#   즉 순간적인 봉우리는 남지만, 그 봉우리가 계속 쌓이는 일은 없어집니다.
#
# [버킷팅] 문장 길이를 일정 배수로 올려서 패딩합니다. 서로 다른 텐서 모양의 개수가 줄어들면
#   "상자 재활용"이 훨씬 잘 됩니다. 배수를 크게 잡을수록 모양은 줄지만 계산 낭비가 커집니다.
#
...
#   512 배수                 7종류                     22.5%
#
#   128 배수면 551종류가 18종류로 줄면서 계산 낭비는 5.6%뿐입니다.
#   512는 모양을 더 줄여주지만 계산 낭비가 4배라 손해입니다. 그래서 128을 골랐습니다.
#   (패딩으로 늘어난 자리는 labels가 -100이라 학습 결과에는 전혀 영향이 없습니다)
#
...
# [메모리 대책] 패딩 길이를 이 값의 배수로 올림 (None이면 끔)
## 배치 안에서 가장 긴 문장에 딱 맞춰 패딩하면 텐서 모양이 551종류나 생겨서
## PyTorch 메모리 캐시가 재활용되지 못하고 계속 불어납니다. (위 [추가 조사] 5번 참고)
## 128의 배수로 올려 맞추면 모양이 18종류로 줄고, 늘어나는 계산량은 5.6%뿐입니다.
...
# =====================================================================================================
# [메모리 대책] 학습 중 주기적으로 GPU 메모리 캐시를 반납하는 콜백
#
# 위 [추가 조사] 3번에서 본 것처럼, PyTorch는 한 번 받아온 GPU 메모리를 자기 캐시에 쌓아두고
# OS에 돌려주지 않습니다. 문장 길이가 계속 신기록을 세우면 "쓸 수 없는 덩어리"만 쌓여서
# 실제 필요량은 6GB인데 드라이버 점유량은 40GB까지 불어납니다.
#
# empty_cache()는 그 캐시를 비우라는 명령입니다. 실측으로 40.5GB -> 7.0GB까지 떨어졌습니다.
#
# 매 스텝 부르면 확실하지만, 버린 메모리를 다시 받아오는 비용 때문에 학습이 느려집니다.
# 그래서 EMPTY_CACHE_EVERY_N_STEPS 마다 한 번만 부릅니다.
#   - 값을 0 이나 None 으로 두면 이 기능이 꺼집니다.
#   - 그래도 메모리가 부족하면 1로 줄여 보세요 (가장 확실하지만 가장 느립니다).
#
# TrainerCallback = Trainer가 학습 도중 정해진 시점마다 불러주는 "콜백" 클래스입니다.
# on_step_end 는 "파라미터 업데이트 1회가 끝난 직후"에 호출됩니다.
# =====================================================================================================
import gc
...
class EmptyCacheCallback(TrainerCallback):
    def on_step_end(self, args, state, control, **kwargs):
        if not EMPTY_CACHE_EVERY_N_STEPS:
            return
        if state.global_step % EMPTY_CACHE_EVERY_N_STEPS:
            return
        ## 파이썬 쪽에서 이미 참조가 끊긴 텐서를 먼저 정리한 뒤에 캐시를 비워야 효과가 있습니다.
        gc.collect()
        if torch.cuda.is_available():
...
if torch.cuda.is_available():
    ...
    trainer = SFTTrainer(
        ...
        args=args, # SFTConfig
        callbacks=[EmptyCacheCallback()], # [메모리 대책] N step마다 GPU 캐시 반납
