...
# 훈련 데이터 793개를 이 모델(Konan-LLM-OND)의 토크나이저로 재보고,
...
#   4                        2,052                   2,052  <- 신기록
#   10                       3,621                   3,621  <- 신기록
#   159                      4,298                   4,298  <- 신기록   (진행표에는 158로 보임)
#   169                      4,450                   4,450  <- 신기록   (진행표에는 168로 보임)
#
...
# -----------------------------------------------------------------------------------------------------
#
#   길이 분포 : min 733 / 중앙값 1,211 / 평균 1,281 / p90 1,752 / p99 2,432 / 최대 4,450
#   상위 12개 : 2,373 2,407 2,428 2,429 2,469 2,554 2,634 2,706 2,919 │ 3,621 4,298 4,450
#                                                                      └ 여기서 뚝 끊깁니다
#
#   길이 초과 기준       샘플 수      비율
#   --------------------------------------
...
#   128 배수로 버킷팅하면 모양이 21 종류인데, 모양마다 활성값 덩어리 한 벌이 캐시에 따로 남습니다.
#   21 x 약 2GB = 약 42GB 로, 위 표의 천장 40.5GB 와 맞아떨어집니다.
#
...
#   켰을 때 40.5GB -> 28.7GB 였습니다. 대신 계산이 20% 늘어납니다)
#
# [가장 단순한 길] 아래 DROP_SAMPLES_LONGER_THAN 을 켜면 유난히 긴 샘플 몇 개를 아예 빼버립니다.
#   이 모델 기준 3072 토큰 초과는 3개뿐이고, 빼고 나면 최대 길이가 2,919 토큰으로 떨어져
...
## 배치마다 가장 긴 문장에 딱 맞춰 패딩하면 텐서 모양이 567종류나 생기고, 모양마다 활성값 덩어리
## 한 벌이 캐시에 따로 남습니다. 128 의 배수로 맞추면 모양이 21종류로 줄고 계산 낭비는 4.9%뿐입니다.
...
#   - 최대 길이가 4608 -> 2,919 토큰으로 떨어져 159 / 169 step의 계단이 통째로 사라집니다.
